...
1. Обидва алгоритми продемонстрували «східчастий» характер покращення результатів. На графіках видно різкі спади значення фітнес-функції (середнього часу очікування), що чергуються з довгими періодами стабільності. Це свідчить про те, що ключовий параметр (ймовірно, ticks-per-cycle) суттєво впливає на пропускну здатність: як тільки алгоритм знаходить вдалий діапазон циклів перемикання світлофора, ефективність системи різко зростає.

2. У цьому експерименті Випадковий пошук (Random Search) виявився ефективнішим за Генетичний алгоритм. Він знайшов рішення з меншим (кращим) показником фітнес-функції — 0.0968 проти 0.1101 у ГА. Випадковий пошук раніше вийшов на своє оптимальне значення (близько 400-го прогону), тоді як ГА досяг свого мінімуму значно пізніше (після 750-го прогону) і зупинився на трохи гіршому локальному оптимумі.

3. Перемога випадкового пошуку у даному випадку пояснюється відносною простотою простору пошуку (змінювалися переважно два параметри: speed-limit та ticks-per-cycle). Для маловимірних задач випадковий перебір часто може «поцілити» в глобальний оптимум швидше, ніж еволюційні методи встигнуть налаштувати популяцію. Генетичний алгоритм «застряг» на обмеженні швидкості 0.3, тоді як Random Search випадково знайшов більш вдале поєднання зі швидкістю 0.2.